...
pub fn setup_vault(path: impl AsRef<std::path::Path>) -> io::Result<()> {
    ...
    debug!("Ensuring vault directory exists: {}", path.display());
...
*",
        ...
        Err(error) if error.kind() == io::ErrorKind::AlreadyExists => {
            debug!(
                "Vault .gitignore already exists: {}",
...
mod tests {
    ...
    #[test]
    fn setup_vault_creates_gitignore_in_existing_directory() {
        let temp_dir = TempDir::new().unwrap();
        let vault_dir = temp_dir.path().join("vault");
        fs::create_dir(&vault_dir).unwrap();

        setup_vault(&vault_dir).unwrap();

        assert_eq!(
            fs::read_to_string(vault_dir.join(".gitignore")).unwrap(),
            "# Automatically generated by Capsula\n*"
        );
    }

    #[test]
    fn setup_vault_preserves_existing_gitignore() {
        let temp_dir = TempDir::new().unwrap();
        let vault_dir = temp_dir.path().join("vault");
        fs::create_dir(&vault_dir).unwrap();
        fs::write(vault_dir.join(".gitignore"), "custom\n").unwrap();
